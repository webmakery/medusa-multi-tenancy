...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const authorization = await authorizeTenantAction(req, res, 'suspend_tenant');
  if (!authorization.allowed) {
    return;
  }

  try {
    const tenant = await tenantManagementService.suspendTenant(tenant_id, authorization.actorEmail);
