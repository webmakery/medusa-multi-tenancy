...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const authorization = await authorizeTenantAction(req, res, 'reactivate_tenant');
  if (!authorization.allowed) {
    return;
  }

  try {
    const tenant = await tenantManagementService.reactivateTenant(tenant_id, authorization.actorEmail);
    return res.status(200).json({ tenant });
  } catch (error: any) {
    return res.status(400).json({ message: error.message || 'Unable to reactivate tenant.' });
  }
}
